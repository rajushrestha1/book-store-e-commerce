...
const NavBar = () => {
  ...
  // Remove "Cart" and "Profile" only if the user is not logged in
  if (isLoggedIn === false) {
    links.splice(2, 2); // Removes "Cart" and "Profile"
  }
    ...
    <div className="flex bg-zinc-800 text-white px-8 py-4 items-center justify-between shadow-lg">
      ...
      {nav && (
        <div className="absolute top-16 left-0 w-full bg-zinc-800 p-6 flex flex-col gap-6 md:hidden">
          ...
          {/* Conditionally render Sign In and Sign Up links for mobile */}
          {!isLoggedIn && (
            <>
              <Link
                to="/signin"
